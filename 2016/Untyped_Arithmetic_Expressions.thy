...
(* changes: theorems command no longer exits in version 2016*)
...
by (induction t u arbitrary: u' rule: eval_NB.induct)
    ((metis eval_NB.cases is_normal_form_NB_def eval1_NB_determinacy)+)
...
(*>*)
(*changes: oneline proof and no more apply*)
theorem eval_NB_always_terminate:
  "\<exists>t'. eval_NB t t' \<and> is_normal_form_NB t'"
by (induction rule: induct_size)
   (meson eval1_NB_impl_eval_NB eval_NB_base eval_NB_transitive eval_once_size_NB is_normal_form_NB_def)
(*<*)
end
(*>*)
