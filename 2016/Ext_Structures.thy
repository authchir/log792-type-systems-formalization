...
primrec shift_L :: "int \<Rightarrow> nat \<Rightarrow> lterm \<Rightarrow> lterm" where
  ...
  "shift_L d c (Tuple L) = Tuple (map (\<lambda>t. shift_L d c t) L)" |
...
primrec subst_L :: "nat \<Rightarrow> lterm \<Rightarrow> lterm \<Rightarrow> lterm" where
  ...
  "susbt_L j s (Tuple L) = Tuple (map (\<lambda>t. subst_L j s t) L)" |
...
inductive eval1_L :: "lterm \<Rightarrow> lterm \<Rightarrow> bool" where
 ...
 -- "Rules relating to evaluation of pairs"
  ...
  eval1_L_Tuple:
    " 1\<le>j\<Longrightarrow> i\<le>length L \<Longrightarrow> is_value_L (Tuple (take (j-1) L)) \<Longrightarrow> 
...
inductive has_type_L :: "lcontext \<Rightarrow> lterm \<Rightarrow> ltype \<Rightarrow> bool" ("((_)/ \<turnstile> (_)/ |:| (_))" [150, 150, 150] 150) where
  ...
  has_type_Pair:
    "\<Gamma> \<turnstile> t1 |:| A \<Longrightarrow> \<Gamma> \<turnstile> t2 |:| B \<Longrightarrow> \<Gamma> \<turnstile> \<lbrace>t1,t2\<rbrace> |:| A |\<times>| B" |
  has_type_Proj1:
    "\<Gamma> \<turnstile> t1 |:| A |\<times>| B \<Longrightarrow> \<Gamma> \<turnstile> \<pi>1 t1 |:| A" |
  has_type_Proj2:
    "\<Gamma> \<turnstile> t1 |:| A |\<times>| B \<Longrightarrow> \<Gamma> \<turnstile> \<pi>2 t1 |:| B" |
  has_type_Tuple:
    "length L = length TL \<Longrightarrow> (\<And>i. 0\<le>i \<Longrightarrow> i< length L \<Longrightarrow> \<Gamma> \<turnstile> (L ! i) |:| (TL ! i))
      \<Longrightarrow> \<Gamma> \<turnstile> Tuple L |:| \<lparr>TL\<rparr>" |
  has_type_Proj:
    "1\<le>i \<Longrightarrow> i\<le> length TL \<Longrightarrow> \<Gamma> \<turnstile> t |:| \<lparr>TL\<rparr> \<Longrightarrow> \<Gamma> \<turnstile> (\<Pi> i t) |:| (TL ! (i-1))"
  
inductive_cases has_type_LetE : "\<Gamma> \<turnstile> Let var x := t1 in t2 |:| B"
inductive_cases has_type_ProjTE: "\<Gamma> \<turnstile> \<Pi> i t |:| R"
...
lemma inversion:
  ...
  "\<Gamma> \<turnstile> \<lbrace>t1,t2\<rbrace> |:| R \<Longrightarrow> \<exists>A B. \<Gamma> \<turnstile> t1 |:| A \<and> \<Gamma> \<turnstile> t2 |:| B \<and> R = A |\<times>| B"
  "\<Gamma> \<turnstile> \<pi>1 t |:| R \<Longrightarrow> \<exists>A B. \<Gamma> \<turnstile> t |:| A |\<times>| B \<and> R = A"
  "\<Gamma> \<turnstile> \<pi>2 t |:| R \<Longrightarrow> \<exists>A B. \<Gamma> \<turnstile> t |:| A |\<times>| B \<and> R = B"
  "\<Gamma> \<turnstile> Tuple L |:| R \<Longrightarrow> \<exists>TL. length L = length TL \<and> (\<forall>i. 0\<le>i \<longrightarrow> i< length L \<longrightarrow> \<Gamma> \<turnstile> (L ! i) |:| (TL ! i)) \<and> R = \<lparr>TL\<rparr>"
  "\<Gamma> \<turnstile> (\<Pi> i t) |:| R \<Longrightarrow> \<exists>TL. R = (TL ! (i-1)) \<and> \<Gamma> \<turnstile> t |:| \<lparr>TL\<rparr> \<and> 1\<le>i \<and> i\<le> length TL"
...
lemma canonical_forms:
  ...
  "is_value_L v \<Longrightarrow> \<Gamma> \<turnstile> v |:| A |\<times>| B \<Longrightarrow> \<exists>v1 v2. is_value_L v1 \<and> is_value_L v2 \<and> v= \<lbrace>v1,v2\<rbrace>"
  "is_value_L v \<Longrightarrow> \<Gamma> \<turnstile> v |:| \<lparr>TL\<rparr> \<Longrightarrow> \<exists>L. is_value_L (Tuple L) \<and> v = Tuple L"
